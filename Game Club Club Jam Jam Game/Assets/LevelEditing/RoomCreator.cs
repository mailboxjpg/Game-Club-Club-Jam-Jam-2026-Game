...
public class RoomCreator : MonoBehaviour
{
    ...
    private void BuildWallSide(Side side, Transform parent)
    {
        ...
        List<(float start, float end)> gaps = openings
            .Where(o => o.side == side)
            .Select(o => (worldOffset: GetWorldCenterOffset(o), o.size))
            .Select(o => (start: o.worldOffset - o.size * 0.5f, end: o.worldOffset + o.size * 0.5f))
            ...
            .Select(g => ClampIntervalToRange(g.start, g.end, 0f, interiorSpan))
            // Shift into wall-local (thickness-extended) coordinates for segment placement
            .Select(g => (start: g.start + wallThickness, end: g.end + wallThickness))
            .Where(g => g.end - g.start > MinSegmentLength)
            .OrderBy(g => g.start)
            .ToList();

        // Merge overlapping gaps so overlapping doors don't create zero/negative-length segments between them
        List<(float start, float end)> mergedGaps = MergeIntervals(gaps);

        // Walk along the wall, emitting a solid segment for every stretch NOT covered by a gap
        float cursor = 0f;
        foreach (var gap in mergedGaps)
        {
            if (gap.start - cursor > MinSegmentLength)
            {
                SpawnWallSegment(side, cursor, gap.start, parent);
            }
            cursor = gap.end;
        }
    ...
    private void SpawnWallSegment(Side side, float offsetStart, float offsetEnd, Transform parent)
    {
        ...
        // If your prefab isn't 1x1, divide by its native size here instead of setting scale directly.
